GREETINGS = "(hi|hello|hi|namaste|good (?:morning|evening|afternoon))+"
CLOSINGS = "(bye|see ya|see you|tata|bye|good(?:night|bye)|good (?:night|bye))+"
HOW_ARE_YOU = "(how [a-z]* (u|you))"
WHATS_YOUR_NAME = "(what is your name|what's your name|tell me your name|your name|people call you)"
WHATS_THE_TIME = "(what time is it|what is the time|tell me the time|current time|now time|time?)"
WHATS_THE_DATE = "((what|which) date is it|what is the date|tell me the date|(current|now|today|today's) date|date?)"
CREATOR = "(who (create|created|code|coded|develop|developed) you|who is your (creator|author|coder))"
YOUTUBE_SEARCH = "(((search|open)( for| about| on| in))*(?=.*(for |about |on |in )*youtube))"
GOOGLE_SEARCH = "((search|open)( for| about| on| in)*|(for |about |on |in )*google)"
WIKI_SEARCH = "(((search|open)( for| about| on| in))*(?=.*(for |about |on |in )*(wikipedia)))"
JOKE = "(a joke|tell me a joke|tell a joke|make me (laugh|smile))"
